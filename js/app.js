...
(function($, owner) {
	...
	db.transaction(function (context) {
		context.executeSql('CREATE TABLE IF NOT EXISTS users (id Integer primary key autoincrement, account,password,sex,age)');
		...
		db.transaction(function (context) {
		   context.executeSql('SELECT * FROM users where account = ? and password = ?', [loginInfo.account,loginInfo.password], function (context, results) {
				...
				if(results.rows.length > 0){
					var id = results.rows[0] ? results.rows[0].id :'';
					return owner.createState(id , callback);
				} else {
	...
	owner.reg = function(regInfo, callback) {
		...
		db.transaction(function (context) {
		   ...
		   context.executeSql('INSERT INTO users (account,password,sex,age) VALUES (?,?,?,?)',[regInfo.account,regInfo.password,regInfo.sex,regInfo.age]);
		 });
...
// 		 db.transaction(function (context) {
// 		    context.executeSql('CREATE TABLE IF NOT EXISTS total (id Integer primary key autoincrement,uid,income,pay)');
// 		    context.executeSql('INSERT INTO lists (uid, income,pay) VALUES (?,?,?)',[uid,addInfo.price,addInfo.type,addInfo.date,addInfo.note]);
// 		 	
// 		  });
	...
	owner.getListDetail = function(id,callback){
		callback = callback || $.noop;
		db.transaction(function (context) {
		   context.executeSql('SELECT * FROM lists where id = ?', [id], function (context, results) {
				console.log(results.rows.length);
				if(results.rows.length > 0){
					return callback(results.rows);
				} 
